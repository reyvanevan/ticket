...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window. approveOrder = async function(orderNumber) {
    ...
    try {
      ...
      // Step 3: Prepare ticket data (QR will be generated by n8n)
      const ticketNumber = `TIX-${orderNumber}-001`;
      
      console.log('📋 Ticket Number:', ticketNumber);
      console.log('📧 Email will be sent to:', order.buyer.email);

      // Step 4: Send email via n8n webhook (QR generated server-side by n8n Code Node)
      let emailSent = false;
      try {
        const emailPayload = {
        ...
        // n8n will generate QR code in Code Node before sending email
